            ...
            Some(response) => {
                ...
                if let Some(found_entry) = find_log_entry("init_pc_amount", log_entries) {
                    ...
                    let tx: EncodedConfirmedTransactionWithStatusMeta =
                        solana_client.get_transaction_with_config(&signature, config)?;
                        ...
                        UiInstruction::Parsed(inside) => {
                            match inside {
                                UiParsedInstruction::PartiallyDecoded(parsed) => {
                                    ...
                                    let base_transfer_instruction :Option<&ParsedInstruction> = find_transfer_instruction_in_inner_instructions_by_destination(&inner_instructions,base_vault,Some(&*TOKEN_PROGRAM_ID.to_string()));
                                    let quote_transfer_instruction = find_transfer_instruction_in_inner_instructions_by_destination(&inner_instructions,quote_vault,Some(&*TOKEN_PROGRAM_ID.to_string()));
